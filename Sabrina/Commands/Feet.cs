            ...
            Parallel.For(0, picsToPost, i =>
             {
                 try
                 {
                     new Thread(() => dep.SankakuBot.PostRandom(channel, i).GetAwaiter().GetResult()).Start();
                 }
                 catch (Exception e)
                 {
                     Console.WriteLine(e);
        ...
        public async Task BoostFeetPics(CommandContext ctx)
        {
            ...
            new Thread(() =>
            {
                for (int i = 0; i < picsToPost; i++)
                {
                    if (i % 2 == 0)
                    {
                        dep.SankakuBot.PostRandom(channel, i).GetAwaiter().GetResult();
                    }
                    else
                    {
                        dep.WaifuJoiBot.PostRandom(channel).GetAwaiter().GetResult();
                    }
                }
            }).Start();
